package com.kuka.CSServer.config;

import com.kuka.CSServer.common.auth.JwtAuthError;
import com.kuka.CSServer.common.auth.JwtAuthFilter;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    @Autowired private JwtAuthError myAuthErrorHandler;

    // 密码明文加密方式配置
    @Bean public PasswordEncoder myEncoder() {
        return new BCryptPasswordEncoder();
    }

    // jwt校验过滤器，从http头部Authorization字段读取token并校验
    @Bean public JwtAuthFilter myAuthFilter() throws Exception {
    ...
    @Override protected void configure(HttpSecurity http) throws Exception {
        ...
        .authorizeRequests()
        // 需要登录
        .antMatchers("/security/hello1").authenticated()
        // 需要角色权限
        .antMatchers("/security/hello2").hasRole("ADMIN")
        // 除上面外的所有请求全部放开
